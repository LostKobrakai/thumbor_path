defmodule ThumborPathTest do
  use ExUnit.Case
  use ExUnitProperties
  doctest ThumborPath

  property "any encoded path can be decoded" do
    check all uri <- ThumborPath.Generators.gen_uri_struct() do
  ...
  property "valid uris stay valid" do
    check all secret <- ThumborPath.Generators.maybe_secret(),
              uri <- ThumborPath.Generators.gen_valid_uri_struct(secret) do
      path = ThumborPath.encode(uri)
      parsed = ThumborPath.parse(path)
